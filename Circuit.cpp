...
void Circuit::addResistor(const string& name, const string& node1, const string& node2, const string& valueStr) {
    if (elementExists(name)) {
        cerr << "Error: Resistor " << name << " already exists in the circuit[cite: 16]." << endl;
        return;
    }
    try {
        ...
        cerr << e.what() << endl;
        cerr << "Correct format: add R<name> <node1> <node2> <value>[cite: 9, 7]." << endl;
...
void Circuit::addCapacitor(const string& name, const string& node1, const string& node2, const string& valueStr) {
    if (elementExists(name)) {
        cerr << "Error: Capacitor " << name << " already exists in the circuit[cite: 33]." << endl;
        return;
    }
    try {
        ...
        cerr << e.what() << endl;
        cerr << "Correct format: add C<name> <node1> <node2> <value>[cite: 25]." << endl;
    } catch (const out_of_range& e) {
        cerr << e.what() << endl;
    }
}
...
void Circuit::addInductor(const string& name, const string& node1, const string& node2, const string& valueStr) {
    if (elementExists(name)) {
        cerr << "Error: Inductor " << name << " already exists in the circuit[cite: 52]." << endl;
        return;
    }
    try {
        ...
        cerr << e.what() << endl;
        cerr << "Correct format: add L<name> <node1> <node2> <value>[cite: 45]." << endl;
    } catch (const out_of_range& e) {
        cerr << e.what() << endl;
    }
}
...
void Circuit::deleteElement(const string& typePrefix, const string& name) {
    ...
    if (it != elements.end()) {
        ...
        if (typePrefix == "R") {
            cerr << "Error: Cannot delete resistor; component not found[cite: 14]." << endl;
        } else if (typePrefix == "C") {
            cerr << "Error: Cannot delete capacitor; component not found[cite: 30]." << endl;
        } else if (typePrefix == "L") {
            cerr << "Error: Cannot delete inductor; component not found[cite: 50]." << endl;
        } else if (typePrefix == "GND") {
            cerr << "Error: Node does not exist or Ground element not found[cite: 56]." << endl;
        }
    }
}
...
void Circuit::renameNode(const string& oldName, const string& newName) {
    ...
    if (!nodeExists(normalizedOldName)) {
        cerr << "ERROR: Node " << normalizedOldName << " does not exist in the circuit[cite: 71]." << endl;
        return;
    }

    if (nodeExists(normalizedNewName)) {
        cerr << "ERROR: Node name " << normalizedNewName << " already exists[cite: 72]." << endl;
        return;
    }
    ...
    cout << "SUCCESS: Node renamed from " << normalizedOldName << " to " << normalizedNewName << "[cite: 73]." << endl;
}